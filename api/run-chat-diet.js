...
} from "./chat-diet-guard.js";

export async function runChatDiet(req, res) {
  if (req.method !== "POST") {
  ...
  const rateKey = auth.userId ? `u:${auth.userId}` : `ip:${getClientIp(req)}`;
  ...
  if (message.length > CHAT_MAX_MESSAGE_LEN) {
    res.status(400).json({ error: `Wiadomość za długa (maks. ${CHAT_MAX_MESSAGE_LEN} znaków).` });
    return;
  }

  const contextJson = JSON.stringify(context);
  if (contextJson.length > CHAT_MAX_CONTEXT_BYTES) {
    res.status(400).json({ error: "Kontekst zapytania jest za duży." });
    return;
  }
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      console.error("[chat-diet] Gemini HTTP", geminiRes.status, String(txt).slice(0, 800));
      res.status(502).json({ error: "Błąd zewnętrznego API (Gemini). Spróbuj ponownie później." });
      return;
    }
    ...
    console.error("[chat-diet]", err);
    res.status(500).json({ error: "Wewnętrzny błąd serwera. Spróbuj ponownie." });
  }
}
